...
void fearMode(ros::Publisher vel_pub) {
	...
	float fear_rot_vel_slow = 0.5;
	int fear_forward_time = 1;
	int rev_cnt = 0;
	ros::Rate loop_rate(LOOP_RATE);
	std::chrono::time_point<std::chrono::system_clock> fear_start_internal;
	...
	while (ros::ok() && !AnyBumperPressed && !FollowingHuman){	
		...
		// go forwards
		fear_start_internal = std::chrono::system_clock::now();
		...
		Vel.linear.x = 0.0;
		Vel.angular.z = fear_rot_vel_slow;
		vel_pub.publish(Vel);
		rotate(vel_pub, 30);
		fear_start_internal = std::chrono::system_clock::now();
